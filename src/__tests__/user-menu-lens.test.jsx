...
describe('role Swap permission gate (BUG-2033 item 3)', function() {
  ...
  it('no portal_manager edit -> role list renders, Swap still renders (Swap no longer gated on portal_manager)', async function() {
    ...
    // Roles section collapses by default; expand it first
    await waitFor(function() { expect(screen.getByText(/Roles on/i)).toBeInTheDocument() })
    fireEvent.click(screen.getByText(/Roles on/i).closest('button'))
