...
function s({ error: e, options: t }) {
	...
	if (e.name === "AbortError") {
		if (t.signal instanceof AbortSignal) return new o({
			message: "Registration ceremony was sent an abort signal",
			code: "ERROR_CEREMONY_ABORTED",
			cause: e
		});
	} else if (e.name === "ConstraintError") {
		if (n.authenticatorSelection?.requireResidentKey === !0) return new o({
			message: "Discoverable credentials were required but no available authenticator supported it",
			code: "ERROR_AUTHENTICATOR_MISSING_DISCOVERABLE_CREDENTIAL_SUPPORT",
			cause: e
		});
		if (t.mediation === "conditional" && n.authenticatorSelection?.userVerification === "required") return new o({
			message: "User verification was required during automatic registration but it could not be performed",
			code: "ERROR_AUTO_REGISTER_USER_VERIFICATION_FAILURE",
			cause: e
		});
		if (n.authenticatorSelection?.userVerification === "required") return new o({
			message: "User verification was required but no available authenticator supported it",
			code: "ERROR_AUTHENTICATOR_MISSING_USER_VERIFICATION_SUPPORT",
			cause: e
		});
	} else if (e.name === "InvalidStateError") return new o({
		message: "The authenticator was previously registered",
		code: "ERROR_AUTHENTICATOR_PREVIOUSLY_REGISTERED",
		cause: e
	});
	else if (e.name === "NotAllowedError") return new o({
		message: e.message,
		code: "ERROR_PASSTHROUGH_SEE_CAUSE_PROPERTY",
		cause: e
	});
	else if (e.name === "NotSupportedError") return n.pubKeyCredParams.filter((e) => e.type === "public-key").length === 0 ? new o({
	...
	} else if (e.name === "UnknownError") return new o({
...
async function d(r) {
	...
	if (!n()) throw Error("WebAuthn is not supported in this browser");
	let l = {
		...a,
		...
		excludeCredentials: a.excludeCredentials?.map(i)
	}, d = {};
	o && (d.mediation = "conditional"), d.publicKey = l, d.signal = c.createNewAbortSignal();
	let p;
	try {
		p = await navigator.credentials.create(d);
	} catch (e) {
		throw s({
			error: e,
			options: d
	...
	if (!p) throw Error("Registration was not completed");
	let { id: m, rawId: h, response: g, type: _ } = p, v;
	typeof g.getTransports == "function" && (v = g.getTransports());
	let y;
	if (typeof g.getPublicKeyAlgorithm == "function") try {
		y = g.getPublicKeyAlgorithm();
	} catch (e) {
		f("getPublicKeyAlgorithm()", e);
	}
	let b;
	if (typeof g.getPublicKey == "function") try {
		let t = g.getPublicKey();
		t !== null && (b = e(t));
	} catch (e) {
		f("getPublicKey()", e);
	}
	let x;
	if (typeof g.getAuthenticatorData == "function") try {
		x = e(g.getAuthenticatorData());
	} catch (e) {
	...
	return {
		...
		rawId: e(h),
		response: {
			attestationObject: e(g.attestationObject),
			clientDataJSON: e(g.clientDataJSON),
			transports: v,
		...
		authenticatorAttachment: u(p.authenticatorAttachment)
	};
}
function f(e, t) {
...
function p() {
	...
	return e?.isConditionalMediationAvailable === void 0 ? m.stubThis(new Promise((e) => e(!1))) : m.stubThis(e.isConditionalMediationAvailable());
}
var m = { stubThis: (e) => e };
//#endregion
...
async function g(r) {
	...
	if (!n()) throw Error("WebAuthn is not supported in this browser");
	let l;
	a.allowCredentials?.length !== 0 && (l = a.allowCredentials?.map(i));
	let d = {
		...a,
		challenge: t(a.challenge),
		allowCredentials: l
	...
	if (o) {
		...
		if (document.querySelectorAll("input[autocomplete$='webauthn']").length < 1 && s) throw Error("No <input> with \"webauthn\" as the only or last value in its `autocomplete` attribute was detected");
		f.mediation = "conditional", d.allowCredentials = [];
	}
	f.publicKey = d, f.signal = c.createNewAbortSignal();
	let m;
	try {
		m = await navigator.credentials.get(f);
	} catch (e) {
		throw h({
			error: e,
			options: f
	...
	if (!m) throw Error("Authentication was not completed");
	let { id: g, rawId: _, response: v, type: y } = m, b;
	return v.userHandle && (b = e(v.userHandle)), {
		id: g,
		rawId: e(_),
		response: {
			authenticatorData: e(v.authenticatorData),
			clientDataJSON: e(v.clientDataJSON),
			signature: e(v.signature),
			userHandle: b
		},
		type: y,
		clientExtensionResults: m.getClientExtensionResults(),
		authenticatorAttachment: u(m.authenticatorAttachment)
	};
}
//#endregion
export { g as startAuthentication, d as startRegistration };
